...
{
  return [self
    sql:@" update file set original=0, toDownload=0 where fileId=? "
    args:@[fileId]
    readOnly:NO];
}
...
reportingPartition:(NSString *)partition
{
  ...
  return @{
    @"totalDeleted": [r2 objectForKey:@"rowsAffected"],
    @"removedFromPartition": removedFromPartition
  };
}
